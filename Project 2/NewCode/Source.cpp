...
namespace
{
    ...
    Camera camera(WINDOW_WIDTH, WINDOW_HEIGHT, glm::vec3(0, 0, -5));
    bool spinning = true;
...
int main(int argc, char* argv[])
{
    ...
    if (!UCreateTexture("../images/remote.png", remoteTexId))
    {
        cout << "Failed to load texture remote.png" << endl;
        return EXIT_FAILURE;
    }
    
    if (!UCreateTexture("../images/fan.png", fanTexId))
    {
        cout << "Failed to load texture fan.png" << endl;
        return EXIT_FAILURE;
    }

    if (!UCreateTexture("../images/pyramid.png", pyramidTexId))
    {
...
const float MAX_OSCILLATE = 1;
const float MAX_SPIN = 10;

float oscillateSpeed = 0;
float spinSpeed = 0;
...
void URender()
{
    ...
    RenderMesh(fanMesh, glm::vec3(4.0f, 4.0f, 1.2f), glm::vec3(0.0f, 45.0f + 40 * glm::sin(glm::radians(fanOscillate)), fanSpin), glm::vec3(2.0f, 5.0f, 2.0f), true, 1);
    ...
    if (spinning)
    {
        if (oscillateSpeed < MAX_OSCILLATE)
            oscillateSpeed += 0.005;
        else
            oscillateSpeed = MAX_OSCILLATE;

        if (spinSpeed < MAX_SPIN)
            spinSpeed += 0.05;
        else
            spinSpeed = MAX_SPIN;
    }
...
void RenderMesh(GLMesh &mesh, glm::vec3 scale, glm::vec3 rotation, glm::vec3 translation, bool useTexture, int texId)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(gProgramId, "view"), 1, GL_FALSE, glm::value_ptr(camera.getViewMatrix()));
    glUniformMatrix4fv(glGetUniformLocation(gProgramId, "projection"), 1, GL_FALSE, glm::value_ptr(camera.getProjectionMatrix()));
    ...
    glUniform3f(glGetUniformLocation(gProgramId, "viewPosition"), camera.pos.x, camera.pos.y, camera.pos.z);
